...
void setup() {
  ...
  fondo = loadImage("fondo.jpg");  // asegúrate del nombre exacto
  fondo.resize(width, height);     // ajusta al tamaño de la ventana
...
void serialEvent(Serial p) {
  ...
  if (bColor == 1) activa.cambiarColor();
  if (bSize == 1) activa.cambiarTam();
  if (bDrop == 1 && !cayendo) {
    cayendo = true;
    myPort.write("GREEN\n");
  }
